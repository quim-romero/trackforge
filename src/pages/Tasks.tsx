...
export default function Tasks() {
  ...
  const filterPadding = density === "compact" ? "py-1 px-2 text-xs" : "py-2 px-3 text-sm";
  ...
  const content = (
    <div className={layoutSpacing}>
      <header className="flex justify-between items-center">
        <div>
          <h2 className="text-2xl font-bold text-gray-900 dark:text-white">Your task flow.</h2>
      ...
      {loading ? (
        <div className={`grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 ${cardGridGap}`}>
          {[...Array(3)].map((_, i) => (
            <div
              key={i}
        ...
        <section className={`grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 ${cardGridGap}`}>
